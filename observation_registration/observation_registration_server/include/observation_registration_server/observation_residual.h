...
struct ObservationResidual {
  ObservationResidual(double* p1, double *p2, double weight) : weight_(weight)
  {
  ...
  template <typename T> bool operator()(const T* const camera_rotation1,
      ...
      T p_c2[3] = {T(p2_[0]), T(p2_[1]), T(p2_[2])};
  ...
  static ceres::CostFunction* Create(double* p1, double* p2, double weight) {
    return (new ceres::AutoDiffCostFunction<
            ObservationResidual, 3, 4, 3>(
                new ObservationResidual(p1,p2, weight)));
  }
