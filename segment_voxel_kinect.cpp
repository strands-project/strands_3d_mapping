...
#include "segment-graph.h" // felzenswalb segmenter
#include "measures.h"
...
typedef boost::graph_traits<Graph>::edge_descriptor Edge;//ADDED
...
void connected_components(std::vector<std::set<size_t> >& groups, const std::set<edge_pair>& edges)
{
    ...
    size_t m = max(it->first, it->second);
...
float graph_cut(std::vector<std::set<size_t> >& groups, const std::set<edge_pair>& edges, const std::map<edge_pair, float>& weights)
{
    std::set<edge_pair>::iterator it = std::max_element(edges.begin(), edges.end(), [](const edge_pair& p1, const edge_pair& p2) {
       return max(p1.first, p1.second) < max(p2.first, p2.second);
    });
                            ...
                            std::set<edge_pair>& remove_pairs)
{
    ...
    for ( ; label_itr != supervoxel_adjacency.end (); )
    {
        ...
        {
            if (false) {//label_itr->first < adjacent_itr->second) {
                continue;
            }
        ...
        /*if (std::find(remove_pairs.begin(), remove_pairs.end(), edge_pair(label_itr->first, label_itr->second)) != remove_pairs.end()) {
            local_pairs.push_back(true);
        }
        else {
            local_pairs.push_back(false);
...
std::pair<bool, float> concave_relationship(pcl::Supervoxel<PointT>::Ptr& first_supervoxel,
                                            pcl::Supervoxel<PointT>::Ptr& second_supervoxel)
{
    pcl::PointCloud<PointT>::Ptr first_voxels = first_supervoxel->voxels_;
    pcl::PointCloud<Normal>::Ptr first_normals = first_supervoxel->normals_;
    pcl::PointCloud<PointT>::Ptr second_voxels = second_supervoxel->voxels_;
    pcl::PointCloud<Normal>::Ptr second_normals = second_supervoxel->normals_;
    ...
    for (size_t i = 0; i < first_voxels->size(); ++i) {
        ...
        pcl::PointCloud<PointT>::iterator it = std::min_element(second_voxels->begin(), second_voxels->end(),
...
void construct_new_edge_set(std::set<edge_pair>& new_edges, const std::vector<std::set<size_t> >& groups, const std::set<edge_pair>& old_edges)
{
...
void global_segmentation(std::vector<std::set<size_t> >& groups,
                         ...
                         const std::map<size_t, size_t>& vertex_points) {
    ...
    for (size_t i = 0; i < connected_groups.size(); ++i) {
        ...
        for (size_t j = 0; j < new_groups.size(); ++j) {
            ...
            //std::set_intersection(new_groups[j].begin(), new_groups[j].end(), connected_groups[i].begin(), connected_groups[i].end(), std::inserter(cut_groups[i], cut_groups[i].begin()));
        }
...
void local_convexity_segmentation(PointCloudT::Ptr& cloud)
{
    std::map <uint32_t, pcl::Supervoxel<PointT>::Ptr > supervoxel_clusters;
    std::multimap<uint32_t, uint32_t> supervoxel_adjacency;
    PointCloudT::Ptr voxel_centroid_cloud;
    ...
    for ( ; label_itr != supervoxel_adjacency.end (); )
    {
        ...
        for ( ; adjacent_itr!=supervoxel_adjacency.equal_range (label_itr->first).second; ++adjacent_itr)
        {
            ...
            //bool is_concave = acos(fabs(sv_normal_cloud->points.at(label_itr->first).getNormalVector3fMap().dot(sv_normal_cloud->points.at(adjacent_itr->second).getNormalVector3fMap()))) > M_PI/8;
            if (is_concave) {
                ...
                /*for (size_t i = 0; i < labels->size(); ++i) {
                    if (labels->at(i).label == label_itr->first || labels->at(i).label == adjacent_itr->second) {
                        colored_voxel_cloud->at(i).r = 255;
    ...
    /*for (edge_pair p : remove_pairs) {
         for (map_iterator it = supervoxel_adjacency.equal_range(p.first).first;
              it != supervoxel_adjacency.equal_range(p.first).second; ++it)
         {
             if (it->second == p.second) {
                supervoxel_adjacency.erase(it);
                break; // maybe not good
             }
         }
    }*/

    std::set<edge_pair> remaining;
    std::vector<std::set<size_t> > groups;
    std::set_difference(edge_pairs.begin(), edge_pairs.end(), remove_pairs.begin(), remove_pairs.end(), std::inserter(remaining, remaining.begin()));
    //graph_cut(groups, edge_pairs, weights);
    global_segmentation(groups, edge_pairs, weights, vertex_points);
    //connected_components(groups, edge_pairs);
...
void segment_objects(CloudColor_t::Ptr& original)
{
    ...
    Cloud_t::Ptr cloud_constrained(new Cloud_t);
    ...
    // outlier removal
    pcl::RadiusOutlierRemoval<Point_t> outrem;
    // build the filter
    ...
    local_convexity_segmentation(cloud_filtered);
}
