...
int main(int argc, char** argv){
	...
	for(int ar = 1; ar < argc; ar++){
		...
		//overall_folder = "/media/johane/SSDstorage/icra_data_surfels/controlled_experiments/object_6_router_box/";
		//overall_folder = "/media/johane/SSDstorage/icra_data_surfels/controlled_experiments/";
	...
	pg->removeAllPointClouds();
				...
				for (unsigned int i=0; i < 2000 && i<object.vAdditionalViews.size(); i+=step){
					...
					cv::waitKey(30);
	//				{
	//					std::cout << poses[i+1] << std::endl << std::endl;
	//					pcl::PointCloud<PointType>::Ptr transformed_cloud (new pcl::PointCloud<PointType> ());
	//					pcl::transformPointCloud (*(av_mask), *transformed_cloud, poses[i+1]);
