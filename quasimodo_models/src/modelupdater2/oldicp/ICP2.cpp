...
std::vector<cv::Mat1w>	depthimages;
...
int main(int argc, char * argv[]){
	...
	while(!asFile.eof()){
		...
		boost::posix_time::time_duration duration11(time.time_of_day());
		unsigned long long int tick = duration11.total_microseconds();
	
		cv::Mat1w depthimg(480, 640);
        uint64_t timestamp = loadDepth(depthimg);
        depthimages.push_back(depthimg);
        timestamps.push_back(timestamp);
        
        time = boost::posix_time::microsec_clock::local_time();
        boost::posix_time::time_duration duration21(time.time_of_day());
        unsigned long long int tock = duration21.total_microseconds();
        
		counter++;
		meanLoad = (float(counter) * meanLoad + (tock - tick) / 1000.0f) / float(counter + 1);

        std::cout << std::setprecision(4) << std::fixed
                  << "\rload data average time: "
                  << meanLoad
                  << "ms ("
                  << 1000.0/meanLoad << " fps)";
                  std::cout.flush();
	...
	std::ofstream file;
	file.open("icp2.poses", std::fstream::out);
	file.close();
	
	reglig::Camera * camera = new reglig::Camera();
	
	for(unsigned int i = 0; i < depthimages.size(); i++){
		cv::Mat1w depthimg = depthimages[i];
...
/*
    ...
    while(!asFile.eof())
    {
        boost::posix_time::ptime time = boost::posix_time::microsec_clock::local_time();
        boost::posix_time::time_duration duration11(time.time_of_day());
        unsigned long long int tick1 = duration11.total_microseconds();
        ...
        time = boost::posix_time::microsec_clock::local_time();
        boost::posix_time::time_duration duration21(time.time_of_day());
        unsigned long long int tock1 = duration21.total_microseconds();
        ...
        meanload = (float(count) * meanload + (tock1 - tick1) / 1000.0f) / float(count + 1);
        meanFast = (float(count) * meanFast + (tock - tick) / 1000.0f) / float(count + 1);
