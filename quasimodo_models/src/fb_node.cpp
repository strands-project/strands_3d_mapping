...
void addFBdata(string path, int start, int stop, int skip){
	...
	//camera->fx			= 528.0;				//Focal Length X
	//camera->fy			= 528.0;				//Focal Length Y
	//camera->cx			= 319.5;				//Center coordinate X
	//camera->cy			= 239.5;				//Center coordinate X
	...
	for(; depth_counter < depth_lines.size(); depth_counter++){
		...
		for(int i = 0; i < rgb_lines.size(); i++){
			double rgbts = rgb_lines.at(i).first;
		...
		//if(diff_best > max_diff){printf("%i %i -> %f %f\n",rgb_counter,depth_counter,rgb_ts,depth_ts);}
	}
	//exit(0);
/*
...
*/
	...
	for(unsigned int i = start; i < rgb_indexes.size() && i < stop; i+= skip){
		...
		if(i <= start+1){printf("depthpath: %s\n",depthpath.c_str());}
		...
		for(unsigned int i = 0; i < 480; i++)
		{
			...
			{
				depthimage.at<unsigned short>(i, j) /= 5;
			}
		}
...
//	addFBdata("/home/johane/Downloads/rgbd_dataset_freiburg3_nostructure_texture_near_withloop", 1, 3, 1);
	//addFBdata(argv[1], 16*30+0, 18*30, 1);
	addFBdata(argv[1], 0, 30000, 1);
	...
	updaters2->saveToFB("testoutput.txt");
	//updaters2->refine();
	//updaters2->saveToFB("testoutput_opt1.txt");
/*
	updaters2->refine();
	updaters2->saveToFB("testoutput_opt2.txt");
	updaters2->refine();
	updaters2->saveToFB("testoutput_opt3.txt");
	updaters2->refine();
	updaters2->saveToFB("testoutput_opt4.txt");
*/
