  ...
  int i;
	...
	if(doTrim)
	{
		...
		for(i = 0; i < inlierNum; i++)
		{
			...
			error += wfunc.getCost(fabs(minDis[i]));
		}
	}
...
//T score	= wfunc.getCost(points[i].dis);
//T weight	= wfunc.getWeight(points[i].dis);
