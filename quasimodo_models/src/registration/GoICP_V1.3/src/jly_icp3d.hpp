...
#ifdef L2
	#define errorcost(X) X*X
	#define errorcost2(X) X
	#define errorweight(X) 1
#endif

#ifdef L1
	#define errorcost(X) X
	#define errorcost2(X) sqrt(X)
	#define errorweight(X) 1.0/(sqrt(X+0.00001))
#endif

#ifdef L05
	#define errorcost(X) sqrt(X)
	#define errorcost2(X) sqrt(sqrt(X))
	#define errorweight(X) 1.0/(sqrt(X+0.00001)*sqrt(sqrt(X+0.00001)))
#endif
  ...
  size_t num;
	...
	for(iter = 0; iter < max_iter; iter++)
	{
		...
		for(i = 0; i < num; i++)
		{
			...
			T score		= errorcost2(points[i].dis);
			T weight	= errorweight(points[i].dis);

			err_new2 += score;

			sum_weight += weight;
			weights.val[i][0] = weight;

			// set model point
			p_m.val[i][0] = model_.pts[points[i].id_model].x; mu_m.val[0][0] += weight*p_m.val[i][0];
			p_m.val[i][1] = model_.pts[points[i].id_model].y; mu_m.val[0][1] += weight*p_m.val[i][1];
			p_m.val[i][2] = model_.pts[points[i].id_model].z; mu_m.val[0][2] += weight*p_m.val[i][2];

			idx = points[i].id_data*3;
			// set query point
			p_d.val[i][0] = r00*data[idx+0] + r01*data[idx+1] + r02*data[idx+2] + t0; mu_d.val[0][0] += weight*p_d.val[i][0];
			p_d.val[i][1] = r10*data[idx+0] + r11*data[idx+1] + r12*data[idx+2] + t1; mu_d.val[0][1] += weight*p_d.val[i][1];
			p_d.val[i][2] = r20*data[idx+0] + r21*data[idx+1] + r22*data[idx+2] + t2; mu_d.val[0][2] += weight*p_d.val[i][2];
...
//		printf("iter: %3.3i ",iter);
//		printf("err_new: %10.10f ",err_new);
//		printf("err_new2: %10.10f\n",err_new2);

		//if(err_new < err_new2){printf("WTF\n");exit(0);}
...
//		Matrix q_t = p_d - Matrix::ones(num,1)*mu_d;
		...
		mu_m = mu_m/sum_weight;
		mu_d = mu_d/sum_weight;
		Matrix q_m = p_m - Matrix::ones(num,1)*mu_m;
		Matrix q_t = p_d - Matrix::ones(num,1)*mu_d;

		for(i = 0; i < num; i++)
		{
			T weight	= weights.val[i][0];
			// set model point
			q_m.val[i][0] *= weight;
			q_m.val[i][1] *= weight;
			q_m.val[i][2] *= weight;
