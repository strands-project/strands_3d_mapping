...
void readConfig(string FName, goicp::GoICP & goicp);
int loadPointCloud(string FName, int & N, goicp::POINT3D **  p);
...
int main(int argc, char** argv)
{
	...
	//for(int i = 0; i < 4; i++){
	for(int i = 0; i < 3; i++){
		int pts = NdDownsampled << 2*i;
		printf("nr points to use = %i\n",pts);
		if(NdDownsampled > 0){goicp.Nd = pts;} // Only use first NdDownsampled data points (assumes data points are randomly ordered)
		
		loadPointCloud(dataFName, pts, &pData);
...
*/
		...
		if(i == 0){
			for(float current_MSEThresh = 1.0; current_MSEThresh >= 0.001; current_MSEThresh *= 0.1){
				goicp.prove_optimal = true;
...
int loadPointCloud(string FName, int & N, goicp::POINT3D ** p)
{
	...
	*p = (goicp::POINT3D *)malloc(sizeof(goicp::POINT3D) * N);
	for(i = 0; i < N; i++)
	{
