...
// "SEEDS2: Superpixels Extracted via Energy-Driven Sampling",
// ECCV 2012
// 
...
SEEDS2::SEEDS2(int width, int height, int nr_channels, int nr_bins)
{
	this->width = width;
	this->height = height;
...
void SEEDS2::initialize(int seeds_w, int seeds_h, int nr_levels)
{
  deinitialize();
  ...
  int nr_SEEDS2 = nr_seeds_w*nr_seeds_h;
  nr_labels[level] = nr_SEEDS2;
  nr_w[level] = nr_seeds_w;
  nr_h[level] = nr_seeds_h;
  labels[level] = new UINT[width*height];
  parent[level] = new UINT[nr_SEEDS2];
  nr_partitions[level] = new UINT[nr_SEEDS2];
  for (level = 1; level < seeds_nr_levels; level++)
  {
	...
	nr_SEEDS2 = nr_seeds_w*nr_seeds_h;
    labels[level] = new UINT[width*height];
	parent[level] = new UINT[nr_SEEDS2];
	nr_partitions[level] = new UINT[nr_SEEDS2];
	nr_labels[level] = nr_SEEDS2;
...
void SEEDS2::update_image_ycbcr(UINT* image) {
...
void SEEDS2::assign_labels()
{
	...
	int step_h = seeds_h;
	int nr_SEEDS2 = nr_seeds_w*nr_seeds_h;
	for (int i=0; i<nr_SEEDS2; i++) nr_partitions[level][i] = 1; // base level: no further partitioning
	for (int y=0; y<height; y++) {
	...
	for (int level = 1; level < seeds_nr_levels; level++)
	{
		...
		step_h *= 2;
		nr_SEEDS2 = nr_seeds_w*nr_seeds_h;
		for (int i=0; i<nr_SEEDS2; i++) nr_partitions[level][i] = 0; // will be added in add_blocks
		for (int y=0; y<height; y++) {
			int label_y = y/step_h;
			if (label_y >= nr_seeds_h) label_y = nr_seeds_h-1;
			const int y_width = y*width;
...
#ifdef LAB_COLORSPACE
void SEEDS2::lab_get_histogram_cutoff_values(const Image& image)
{
...
void SEEDS2::compute_histograms(int until_level)
{
...
// Merging Algorithm
void SEEDS2::update_blocks(int level, float req_confidence)
{
	int labelA;
	int labelB;
	int sublabel;
...
// Border Updating Algorithm
void SEEDS2::update_pixels_means()
{
	int labelA;
	int labelB;
	int priorA=0;
...
void SEEDS2::update(int level, int label_new, int x, int y)
{
...
void SEEDS2::add_pixel(int level, int label, int x, int y)
{
	histogram[level][label][image_bins[y*width+x]]++;
	T[level][label]++;
}

void SEEDS2::add_pixel_m(int level, int label, int x, int y)
{
	histogram[level][label][image_bins[y*width+x]]++;
	T[level][label]++;
...
void SEEDS2::delete_pixel(int level, int label, int x, int y)
{
	histogram[level][label][image_bins[y*width+x]]--;
	T[level][label]--;
}

void SEEDS2::delete_pixel_m(int level, int label, int x, int y)
{
	histogram[level][label][image_bins[y*width+x]]--;
	T[level][label]--;
...
void SEEDS2::add_block(int level, int label, int sublevel, int sublabel)
{
...
void SEEDS2::delete_block(int level, int label, int sublevel, int sublabel)
{
...
void SEEDS2::update_labels(int level)
{
	for (int i=0; i<width*height; i++)
	{
...
bool SEEDS2::probability(int color, int label1, int label2, int prior1, int prior2)
{
...
bool SEEDS2::probability_means(float L, float a, float b, int label1, int label2, int prior1, int prior2)
{
...
void SEEDS2::LAB2RGB(float L, float a, float b, int* R, int* G, int* B)
{
	float T1 = 0.008856;
	float T2 = 0.206893;
...
int SEEDS2::RGB2LAB(const int& r, const int& g, const int& b, float* lval, float* aval, float* bval)
{
...
int SEEDS2::RGB2HSV(const int& r, const int& g, const int& b, float* hval, float* sval, float* vval)
{
	float r_ = r / 256.0;
	float g_ = g / 256.0;
	float b_ = b / 256.0;
...
int SEEDS2::LAB2bin(float l, float a, float b)
{
	// binning
...
int SEEDS2::RGB2LAB_special(int r, int g, int b, float* lval, float* aval, float* bval)
{
	float xVal = 0.412453 * r + 0.357580 * g + 0.180423 * b;
	float yVal = 0.212671 * r + 0.715160 * g + 0.072169 * b;
	float zVal = 0.019334 * r + 0.119193 * g + 0.950227 * b;
...
int SEEDS2::RGB2LAB_special(int r, int g, int b, int* bin_l, int* bin_a, int* bin_b)
{
	float xVal = 0.412453 * r + 0.357580 * g + 0.180423 * b;
	float yVal = 0.212671 * r + 0.715160 * g + 0.072169 * b;
	float zVal = 0.019334 * r + 0.119193 * g + 0.950227 * b;
...
int SEEDS2::threebythree(int x, int y, UINT label)
{
	int count = 0;
...
int SEEDS2::threebyfour(int x, int y, UINT label)
{
	int count = 0;
	const UINT* ptr_label = labels[seeds_top_level];
	const int y_width = y*width;
...
int SEEDS2::fourbythree(int x, int y, UINT label)
{
	int count = 0;
	const UINT* ptr_label = labels[seeds_top_level];
	const int y_width = y*width;
...
float SEEDS2::intersection(int level1, int label1, int level2, int label2)
{
...
bool SEEDS2::check_split(int a11, int a12, int a13, int a21, int a22, int a23, int a31, int a32, int a33, bool horizontal, bool forward)
{
	if (horizontal)
	{
		if (forward)
...
void SEEDS2::SaveLabels_Text(string filename)
{
