...
SEEDS::SEEDS(int width, int height, int nr_channels, int nr_bins, int normal_nr_bins)
{
	this->width = width;
	this->height = height;
	this->nr_channels = nr_channels;
	this->nr_bins = nr_bins;
    this->normal_nr_bins = normal_nr_bins;
    ...
    normal_bins = new UINT[width*height];
    normal_x = new float[width*height];
    normal_y = new float[width*height];
    normal_z = new float[width*height];
    normal_histogram_size = 1 + normal_nr_bins*normal_nr_bins*normal_nr_bins;


    smoothing_prior = 1.0;
    smoothing_power = 3.0;

    xydistance_prior = 1.0;
    xydistance_power = 2.0;

    colorhist_weight = 1.0;
    normalhist_weight = 1.0;
...
void SEEDS::initialize(int seeds_w, int seeds_h, int nr_levels)
{
...
#endif
    X_channel = new float[nr_labels[seeds_top_level]];
    Y_channel = new float[nr_labels[seeds_top_level]];
  ...
  // create histogram buffers
  normal_histogram = new int**[seeds_nr_levels]; // histograms are kept for each level and each label [level][label][bin]
  normal_T = new int*[seeds_nr_levels]; // block sizes are kept at each level [level][label]
  for (int level=0; level<seeds_nr_levels; level++)
  {
    normal_histogram[level] = new int*[nr_labels[level]]; // histograms are kept for each level and each label [level][label][bin]
    normal_T[level] = new int[nr_labels[level]]; // block sizes are kept at each level [level][label]
    for (UINT label=0; label<nr_labels[level]; label++)
    {
      normal_histogram[level][label] = new int[histogram_size]; // histogram bins
    }
  }
...
void SEEDS::update_image_ycbcr(UINT* image, float * normal) {
...
#ifdef normal_COLORSPACE
	  image_bins[i] = normal2bin(nx,ny,nz,nr_bins);
	  image_l[i] = nx;
	  image_a[i] = ny;
	  image_b[i] = nz;
#endif

	  normal_bins[i] = normal2bin(nx,ny,nz,normal_nr_bins);
      normal_x[i] = nx;
      normal_y[i] = ny;
      normal_z[i] = nz;
    }
...
{
        unsigned char * cimage = (unsigned char *)image;
...
void SEEDS::compute_histograms(int until_level)
{
    ...
    for (int level=0; level<seeds_nr_levels; level++){
        for (UINT label=0; label<nr_labels[level]; label++){
          memset(normal_histogram[level][label], 0, sizeof(int)*normal_histogram_size);
        }
        memset(normal_T[level], 0, sizeof(int)*nr_labels[level]);
    }
			...
			for (int x=1; x<width-2; x++) 
			{
				...
				if (labelA != labelB)
				{
					if (!check_split(a11, a12, a13, a21, a22, a23, a31, a32, a33, true, true))
					{
                        ...
                        if (probability(image_bins[y*width+x],normal_bins[y*width+x], labelA, labelB, priorA, priorB))
						{
							update(seeds_top_level, labelB, x, y);
						}
						else if (!check_split(a12, a13, a14, a22, a23, a24, a32, a33, a34, true, false))
						{
                            if (probability(image_bins[y*width+x+1],normal_bins[y*width+x+1], labelB, labelA, priorB, priorA))
							{
			...
			for (int y=1; y<height-2; y++)
			{
				...
				if (labelA != labelB)
				{
					if (!check_split(a11, a12, a13, a21, a22, a23, a31, a32, a33, false, true))
					{
						...
						{
                            if (probability(image_bins[(y+1)*width+x],normal_bins[(y+1)*width+x], labelB, labelA, priorB, priorA))
							{
...
void SEEDS::add_pixel(int level, int label, int x, int y)
{
    ...
    int nnb = normal_bins[y*width+x];
    if(nnb > 0){
        normal_histogram[level][label][nnb]++;
        normal_T[level][label]++;
    }
}
...
void SEEDS::add_block(int level, int label, int sublevel, int sublabel)
{
    ...
    for (int n=0; n<normal_histogram_size; n++){
        normal_histogram[level][label][n] += normal_histogram[sublevel][sublabel][n];
    }
    normal_T[level][label] += normal_T[sublevel][sublabel];
...
int SEEDS::normal2bin(float nx, float ny, float nz, int bins){
    if(nx > 1){return 0;}

    float nx2 = 0.5*(nx+1.0);
    float ny2 = 0.5*(ny+1.0);
    float nz2 = 0.5*(nz+1.0);
	int xbin = int(nx2 * float(bins));
	int ybin = int(ny2 * float(bins));
	int zbin = int(nz2 * float(bins));

	if(xbin == bins) --xbin;
	if(ybin == bins) --ybin;
	if(zbin == bins) --zbin;

    //printf("normal_nr_bins: %i\n",normal_nr_bins);
    //printf("%f %f %f -> %f %f %f -> %i %i %i -> %i\n",nx,ny,nz,nx2,ny2,nz2,xbin,ybin,zbin,1 + xbin + normal_nr_bins*(ybin + normal_nr_bins*zbin));
    //exit(0);
	return 1 + xbin + bins*(ybin + bins*zbin);
}
...
float SEEDS::threebythree(int x, int y, UINT label)
{
    float count = 0;
    ...
    float Xmean = X_channel[label] / T[seeds_top_level][label];
    float Ymean = Y_channel[label] / T[seeds_top_level][label];
    float dx = Xmean - float(x);
    float dy = Ymean - float(y);

    double smoothing_component = smoothing_prior*pow(count,smoothing_power);
    double distance_component = pow(xydistance_prior,xydistance_power)*pow(0.01+dx*dx+dy*dy,-xydistance_power);
    return smoothing_component+distance_component;
}

float SEEDS::threebyfour(int x, int y, UINT label)
{
    float count = 0;
	const UINT* ptr_label = labels[seeds_top_level];
	const int y_width = y*width;
    ...
    double distance_weight = pow(xydistance_prior,xydistance_power);
    double distance = sqrt(dx*dx+dy*dy);
    double reweighted = pow(0.01+distance,-xydistance_power);
    double distance_component = distance_weight*reweighted;
    if((x % 128 == 0) && (y % 96 == 0)){
        printf("count: %f -> smoothing: %f\n",count,smoothing_component);
        printf("Xmean: %f Ymean: %f dx: %f %f -> distance_weight: %f distance: %f reweighted: %f compact: %f\n",Xmean,Ymean,dx,dy,distance_weight,distance,reweighted,distance_component);

        //printf("Xmean: %f Ymean: %f dx: %f %f count: %f -> smoothing: %f : distance: %f compact: %f\n",Xmean,Ymean,dx,dy,count,smoothing_component,distance_component);
    }

    return smoothing_component+distance_component;
}

float SEEDS::fourbythree(int x, int y, UINT label)
{
    float count = 0;
	const UINT* ptr_label = labels[seeds_top_level];
	const int y_width = y*width;
...
#ifdef normal_COLORSPACE
	for (int n=1; n<histogram_size; n++){
...
bool SEEDS::probability(int color,int normal, int label1, int label2, float prior1, float prior2)
{
/*
#ifdef normal_COLORSPACE
	float P_label1 = 1;
	...
	float N_label2 = 1;
	if(normal != 0){
		float P_label1 = (float)histogram[seeds_top_level][label1][color] / std::max(1.0f,(float)T[seeds_top_level][label1]);
		float P_label2 = (float)histogram[seeds_top_level][label2][color] / std::max(1.0f,(float)T[seeds_top_level][label2]);

		float N_label1 = (float)normal_histogram[seeds_top_level][label1][normal] / (0.1 + (float)normal_T[seeds_top_level][label1]);
		float N_label2 = (float)normal_histogram[seeds_top_level][label2][normal] / (0.1 + (float)normal_T[seeds_top_level][label2]);
	}
#else
	*/
...
*/
		...
		N_label1 *= (float) prior1;
		N_label2 *= (float) prior2;
	#endif

	return ((P_label2*colorhist_weight + N_label2*normalhist_weight) > (P_label1*colorhist_weight + N_label1*normalhist_weight));
}

bool SEEDS::probability_means(float L, float a, float b, int label1, int label2, float prior1, float prior2)
{
